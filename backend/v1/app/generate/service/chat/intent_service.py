...
def classify_entry_intent(content: str) -> dict:
    ...
    try:
        ...
        return _fallback_entry_intent(content)
...
def _fallback_entry_intent(content: str) -> dict:
    ...
    if not text:
        return {"should_create_project": False, "reason": "空消息"}

    if re.search(r"https?://\S+", text):
        return {"should_create_project": True, "reason": "包含URL"}
    ...
    if has_creation and has_video:
        return {"should_create_project": True, "reason": "包含创建和视频关键词"}

    return {"should_create_project": False, "reason": "无创建意图"}
...
- EDIT_FRAME需要在modifications中指定要修改的字段和新值
...
def classify_project_intent(
    ...
    整合 llm_agent_service.plan() 和 workflow_agent_service.plan()
    ...
    try:
        ...
        return _normalize_plan(plan, frames)
    except Exception as e:
