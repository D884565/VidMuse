...
class FactorService:
    ...
    @staticmethod
    async def create_factor(db: AsyncSession, factor_data: Dict[str, Any]) -> Dict[str, Any]:
        """创建因子
        ...
        if not factor_data.get("factor_id"):
            ...
            # 检查factor_id是否已存在
            existing = await FactorDAO.get_factor_by_factor_id(db, factor_data["factor_id"])
            if existing:
                raise BusinessException(RESOURCE_ALREADY_EXISTS, "因子ID已存在")

        factor = await FactorDAO.create_factor(db, factor_data)
        return FactorService._factor_to_dict(factor)

    @staticmethod
    async def get_factor(db: AsyncSession, factor_id: int) -> Dict[str, Any]:
        """获取因子详情
        ...
        """
        factor = await FactorDAO.get_factor_by_id(db, factor_id)
        if not factor:
            raise BusinessException(RESOURCE_NOT_FOUND, "因子不存在")
        return FactorService._factor_to_dict(factor)

    @staticmethod
    async def get_factor_by_factor_id(db: AsyncSession, factor_id: str) -> Optional[Dict[str, Any]]:
        """根据全局唯一factor_id查询因子

        :param db: 数据库会话
        :param factor_id: 全局唯一因子ID
        :return: 因子信息，不存在返回None
        """
        factor = await FactorDAO.get_factor_by_factor_id(db, factor_id)
        return FactorService._factor_to_dict(factor) if factor else None

    @staticmethod
    async def update_factor(db: AsyncSession, factor_id: int, update_data: Dict[str, Any]) -> Dict[str, Any]:
        """更新因子信息
        ...
        if "factor_id" in update_data and update_data["factor_id"] != factor.factor_id:
            existing = await FactorDAO.get_factor_by_factor_id(db, update_data["factor_id"])
            if existing:
                raise BusinessException(RESOURCE_ALREADY_EXISTS, "因子ID已存在")

        updated_factor = await FactorDAO.update_factor(db, factor_id, update_data)
    ...
    async def delete_factor(db: AsyncSession, factor_id: int) -> None:
        ...
        # 检查因子是否被模板引用
        relations = await TemplateFactorRelationDAO.get_relations_by_factor_id(db, factor.factor_id)
        if relations:
            raise BusinessException(PARAM_ERROR, "该因子已被模板引用，无法删除")

        success = await FactorDAO.delete_factor(db, factor_id)
        if not success:
            raise BusinessException(RESOURCE_NOT_FOUND, "因子不存在")
    ...
    async def list_factors(
        ...
        :return: 分页结果
        """
        total, factors = await FactorDAO.list_factors(
...
class StrategyService:
    ...
    @staticmethod
    async def create_strategy(db: AsyncSession, strategy_data: Dict[str, Any]) -> Dict[str, Any]:
        """创建策略
        ...
        if not strategy_data.get("strategy_id"):
            ...
            # 检查strategy_id是否已存在
            existing = await StrategyDAO.get_strategy_by_strategy_id(db, strategy_data["strategy_id"])
            if existing:
                raise BusinessException(RESOURCE_ALREADY_EXISTS, "策略ID已存在")

        strategy = await StrategyDAO.create_strategy(db, strategy_data)
        return StrategyService._strategy_to_dict(strategy)

    @staticmethod
    async def get_strategy(db: AsyncSession, strategy_id: int) -> Dict[str, Any]:
        """获取策略详情
        ...
        """
        strategy = await StrategyDAO.get_strategy_by_id(db, strategy_id)
        if not strategy:
            raise BusinessException(RESOURCE_NOT_FOUND, "策略不存在")
        return StrategyService._strategy_to_dict(strategy)

    @staticmethod
    async def get_strategy_by_strategy_id(db: AsyncSession, strategy_id: str) -> Optional[Dict[str, Any]]:
        """根据全局唯一strategy_id查询策略

        :param db: 数据库会话
        :param strategy_id: 全局唯一策略ID
        :return: 策略信息，不存在返回None
        """
        strategy = await StrategyDAO.get_strategy_by_strategy_id(db, strategy_id)
        return StrategyService._strategy_to_dict(strategy) if strategy else None

    @staticmethod
    async def update_strategy(db: AsyncSession, strategy_id: int, update_data: Dict[str, Any]) -> Dict[str, Any]:
        """更新策略信息
        ...
        if "strategy_id" in update_data and update_data["strategy_id"] != strategy.strategy_id:
            existing = await StrategyDAO.get_strategy_by_strategy_id(db, update_data["strategy_id"])
            if existing:
                raise BusinessException(RESOURCE_ALREADY_EXISTS, "策略ID已存在")

        updated_strategy = await StrategyDAO.update_strategy(db, strategy_id, update_data)
    ...
    async def delete_strategy(db: AsyncSession, strategy_id: int) -> None:
        ...
        # 检查策略是否被模板引用
        templates = await InspirationTemplateDAO.list_templates(db, strategy_id=strategy.strategy_id)
        if templates[0] > 0:
            raise BusinessException(PARAM_ERROR, "该策略已被模板引用，无法删除")

        success = await StrategyDAO.delete_strategy(db, strategy_id)
        if not success:
            raise BusinessException(RESOURCE_NOT_FOUND, "策略不存在")
    ...
    async def list_strategies(
        ...
        :return: 分页结果
        """
        total, strategies = await StrategyDAO.list_strategies(
...
class InspirationTemplateService:
    ...
    @staticmethod
    async def create_template(db: AsyncSession, template_data: Dict[str, Any]) -> Dict[str, Any]:
        """创建模板
        ...
        if not template_data.get("template_id"):
            ...
            # 检查template_id是否已存在
            existing = await InspirationTemplateDAO.get_template_by_template_id(db, template_data["template_id"])
            if existing:
        ...
        # 返回完整的模板信息
        return await InspirationTemplateService.get_template_detail(db, template.id)

    @staticmethod
    async def get_template(db: AsyncSession, template_id: int) -> Dict[str, Any]:
        """获取模板基本信息
        ...
        """
        template = await InspirationTemplateDAO.get_template_by_id(db, template_id)
        if not template:
            raise BusinessException(RESOURCE_NOT_FOUND, "模板不存在")
        return InspirationTemplateService._template_to_dict(template)

    @staticmethod
    async def get_template_by_template_id(db: AsyncSession, template_id: str) -> Optional[Dict[str, Any]]:
        """根据全局唯一template_id查询模板

        :param db: 数据库会话
        :param template_id: 全局唯一模板ID
        :return: 模板基本信息，不存在返回None
        """
        template = await InspirationTemplateDAO.get_template_by_template_id(db, template_id)
        return InspirationTemplateService._template_to_dict(template) if template else None

    @staticmethod
    async def get_template_detail(db: AsyncSession, template_id: int) -> Dict[str, Any]:
        """获取模板详细信息（包含关联的策略和因子）
        ...
        # 查询关联的策略
        strategy = await StrategyDAO.get_strategy_by_strategy_id(db, template.strategy_id)
        if strategy:
            result["strategy"] = StrategyService._strategy_to_dict(strategy)

        # 查询关联的因子
        relations = await TemplateFactorRelationDAO.get_relations_by_template_id(db, template.template_id)
        required_factors = []
        optional_factors = []
    ...
    @staticmethod
    async def update_template(db: AsyncSession, template_id: int, update_data: Dict[str, Any]) -> Dict[str, Any]:
        """更新模板信息
        ...
        if "strategy_id" in update_data and update_data["strategy_id"] != template.strategy_id:
            strategy = await StrategyDAO.get_strategy_by_strategy_id(db, update_data["strategy_id"])
            if not strategy:
        ...
        if "template_id" in update_data and update_data["template_id"] != template.template_id:
            existing = await InspirationTemplateDAO.get_template_by_template_id(db, update_data["template_id"])
            if existing:
                raise BusinessException(RESOURCE_ALREADY_EXISTS, "模板ID已存在")

        # 更新模板基本信息
        updated_template = await InspirationTemplateDAO.update_template(db, template_id, update_data)

        # 如果提供了factor_relations，全量更新关联
        if factor_relations is not None:
            # 先删除所有旧的关联
            await TemplateFactorRelationDAO.delete_relations_by_template_id(db, updated_template.template_id)
        ...
        # 返回完整的模板信息
        return await InspirationTemplateService.get_template_detail(db, template_id)
    ...
    async def delete_template(db: AsyncSession, template_id: int) -> None:
        ...
        # 删除关联的因子关系
        await TemplateFactorRelationDAO.delete_relations_by_template_id(db, template.template_id)

        # 删除模板
        success = await InspirationTemplateDAO.delete_template(db, template_id)
        if not success:
            raise BusinessException(RESOURCE_NOT_FOUND, "模板不存在")
    ...
    async def list_templates(
        ...
        :return: 分页结果
        """
        total, templates = await InspirationTemplateDAO.list_templates(
        ...
        for t in templates:
            ...
            # 查询关联的策略名称
            strategy = await StrategyDAO.get_strategy_by_strategy_id(db, t.strategy_id)
            if strategy:
...
class TemplateFactorRelationService:
    ...
    @staticmethod
    async def add_relation(db: AsyncSession, relation_data: Dict[str, Any]) -> Dict[str, Any]:
        """添加模板-因子关联
        ...
        # 检查关联是否已存在
        existing_relations = await TemplateFactorRelationDAO.get_relations_by_template_id(db, template_id)
        for rel in existing_relations:
            if rel.factor_id == factor_id:
                raise BusinessException(RESOURCE_ALREADY_EXISTS, "该因子已关联到模板")

        relation = await TemplateFactorRelationDAO.create_relation(db, relation_data)
        return TemplateFactorRelationService._relation_to_dict(relation)

    @staticmethod
    async def update_relation(db: AsyncSession, relation_id: int, update_data: Dict[str, Any]) -> Dict[str, Any]:
        """更新关联信息
        ...
        """
        relation = await TemplateFactorRelationDAO.get_relation_by_id(db, relation_id)
        if not relation:
            raise BusinessException(RESOURCE_NOT_FOUND, "关联不存在")

        updated_relation = await TemplateFactorRelationDAO.update_relation(db, relation_id, update_data)
    ...
    async def delete_relation(db: AsyncSession, relation_id: int) -> None:
        ...
        success = await TemplateFactorRelationDAO.delete_relation(db, relation_id)
        if not success:
            raise BusinessException(RESOURCE_NOT_FOUND, "关联不存在")

    @staticmethod
    async def get_template_factors(db: AsyncSession, template_id: str) -> Dict[str, Any]:
        """获取模板关联的所有因子
        ...
        relations = await TemplateFactorRelationDAO.get_relations_by_template_id(db, template_id)
        required_factors = []
        optional_factors = []
