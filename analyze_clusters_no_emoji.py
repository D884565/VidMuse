...
class ClusterAnalyzer:
    ...
    def _fetch_and_cluster(self, collection_name: str) -> Dict[int, List[Dict[str, Any]]]:
        ...
        for label in unique_labels:
            ...
            if len(cluster_docs) > self.max_samples_per_cluster:
                ...
                print(f"   簇 {int(label)}: 样本数{len(indices)}，筛选前{self.max_samples_per_cluster}个进行分析")
            else:
                print(f"   簇 {int(label)}: 样本数{len(indices)}")
    ...
    async def analyze_video_clusters(self):
        ...
        for cluster_id, docs in self.video_clusters.items():
            ...
            try:
                ...
                print(f"        核心逻辑: {strategy.core_logic[:100]}...")
    ...
    def generate_simple_report(self, output_file: str = None):
        """生成简单的统计报告"""
        if not output_file:
            output_file = f"cluster_analysis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write("统计概览:\n")
            f.write(f"  slice簇数量: {self.stats['slice_clusters']}\n")
            f.write(f"  video簇数量: {self.stats['video_clusters']}\n")
            f.write(f"  总簇数量: {self.stats['slice_clusters'] + self.stats['video_clusters']}\n")
            f.write(f"  生成因子总数: {self.stats['factors_generated']}\n")
            f.write(f"  生成策略总数: {self.stats['strategies_generated']}\n")
            f.write(f"  生成模板总数: {self.stats['templates_generated']}\n")
            f.write(f"  分析失败簇数量: {len(self.stats['failed_clusters'])}\n")
            f.write(f"  成功率: {(self.stats['slice_clusters'] + self.stats['video_clusters'] - len(self.stats['failed_clusters'])) / (self.stats['slice_clusters'] + self.stats['video_clusters']) * 100:.1f}%\n\n")

            f.write("因子列表:\n")
            for factor_id, factor in self.factors.items():
                f.write(f"  [{factor_id}] {factor.name} ({factor.factor_type})\n")
                f.write(f"    描述: {factor.description[:100]}...\n\n")

            f.write("\n策略列表:\n")
            for strategy_id, strategy in self.strategies.items():
                f.write(f"  [{strategy_id}] {strategy.name}\n")
                f.write(f"    成功率: {strategy.success_rate:.2f}\n")
                f.write(f"    核心逻辑: {strategy.core_logic[:150]}...\n\n")

            f.write("\n模板列表:\n")
            for template in self.templates:
                f.write(f"  [{template.template_id}] {template.name}\n")
                f.write(f"    关联策略: {template.strategy.name}\n")
                f.write(f"    成功率: {template.success_rate:.2f}\n")
                f.write(f"    必填因子: {len(template.required_factors)} 个\n")
                f.write(f"    可选因子: {len(template.optional_factors)} 个\n\n")
...
async def main():
    ...
    # 5. 生成报告
    report_file = analyzer.generate_simple_report()
