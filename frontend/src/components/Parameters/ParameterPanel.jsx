  ...
  return (
    <div className="absolute bottom-full right-0 mb-3 w-[520px] rounded-xl border border-[rgba(124,58,237,0.2)] bg-[rgba(26,26,46,0.95)] p-5 shadow-[0_4px_24px_rgba(124,58,237,0.15)] backdrop-blur-xl">
      ...
      <AspectRatioSelector
        ...
        onChange={(aspectRatio) => updateParameters({ aspectRatio })}
      />

      <DurationSlider
        value={parameters.duration}
        onChange={(duration) => updateParameters({ duration })}
      />

      <div className="mt-5">
        <p className="mb-3 text-xs text-[var(--text-muted)]">画质 / 风格</p>
        <div className="flex flex-wrap gap-2">
          {styles.map((style) => (
            <button
              key={style}
              className={`rounded-lg border px-3 py-2 text-sm ${
                parameters.quality === style
              ...
              type="button"
              onClick={() => updateParameters({ quality: style })}
            >
              {style}
            </button>
          ))}
